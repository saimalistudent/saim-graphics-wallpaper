/**
 * Restore original (uncompressed) Drive PDFs to Supabase CDN.
 * Replaces lossy "compressed.pdf" objects with full-quality originals.
 *
 * Usage: npx tsx scripts/restore-original-pdfs.ts
 * Optional: LIMIT=3 npx tsx scripts/restore-original-pdfs.ts
 */
...
async function main() {
  ...
  const { data: rows, error } = await supabase
    .from("catalogs")
    .select("id, title, drive_file_id, pdf_path, pdf_bytes")
    .order("title");
  if (error) throw error;

  let list = (rows ?? []).filter((r) => r.drive_file_id);
  // Default: only rewrite still-lossy compressed objects (resume-safe)
  if (process.env.ALL !== "1") {
    list = list.filter((r) => (r.pdf_path || "").includes("compressed"));
  }
  if (limit > 0) list = list.slice(0, limit);
  console.log(`Restoring ${list.length} original PDFs from Drive/cache…`);

  let ok = 0;
  let fail = 0;

  for (const row of list) {
    const tmpFile = path.join(TMP, `restore-${row.id}.pdf`);
    ...
    try {
      ...
      if (!isPdf(fileBuf)) throw new Error("Not a PDF");

      const objectPath = `catalogs/${row.id}/original-${Date.now()}.pdf`;
      ...
      if (row.pdf_path && row.pdf_path !== objectPath) {
        await supabase.storage.from(PDF_BUCKET).remove([row.pdf_path]);
      }
